...
{{if .Easy}}func New{{.Type}}ModelV2() {{.Type}}Model {
    return &custom{{.Type}}Model{
        default{{.Type}}Model: newDefault{{.Type}}Model(nil),
    }
}{{else}}func New{{.Type}}ModelV2() {{.Type}}Model {
    return &custom{{.Type}}Model{
        default{{.Type}}Model: newDefault{{.Type}}Model(nil),
    }
}{{end}}
...
func (model (*custom{{.Type}}Model)) DropTable() {
	mgm.Coll(&{{.Type}}{}).Drop(context.Background())
}